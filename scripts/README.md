...
```bash
cd /home/zcx/CLionProjects/rx-met
...
- AIMET ONNX custom op 的最小 CPU 校准和推理；
- 原生动态库是否存在未解析依赖；
- 使用 `--gpu` 时的 AIMET v2 CUDA 流程；
- 使用 `--gpu` 时的 QuantGRU CUDA forward、backward 和同步。

这属于镜像冒烟验收，不会读取正式 Speech Commands 数据集，也不会运行
`/home/zcx/CLionProjects/unisoc-model-example` 的完整 ONNX PTQ example。完整数据集
验证仍需在目标 GPU 服务器单独执行。
...
当前环境安装的是 CPU 版 ONNX Runtime，因此该脚本不占用 GPU；它仍会在每个指定
产品镜像内运行，以检查各变体的完整依赖。模型输入名和静态 shape 必须与校准样本
匹配。多输入模型每个 NPZ 需包含所有输入，或使用 example 支持的同样本 NPY 命名。
...
脚本在本地临时目录执行 `docker save`、zstd 压缩和 SHA-256 校验，全部成功后才
放入正式本地输出目录。它明确禁止输出到 `/mnt/data2`。向共享盘或其他机器复制、
移动文件必须由维护人员人工完成。
...
主要检查镜像平台、环境角色、环境版本、CUDA/Torch 版本和 `pip check`。build-env
...
```text
aimet_common/_libpymo.cpython-310-x86_64-linux-gnu.so
aimet_common/libquant_info.cpython-310-x86_64-linux-gnu.so
...
CPython 3.10 的 `quant_gru` wheel。没有 `nvcc` 或使用 CPU Torch 时会明确失败。
...
普通 rx-met、AIMET、QuantGRU 或 examples 修改不需要升级环境版本。CUDA、Torch、
Python、Ubuntu、requirements lock 或工具链发生变化时，应创建 `deps-v2`，不要
覆盖已经交付的 `deps-v1`：
...
- 所有导出脚本只在项目本地生成文件，不自动复制或移动到共享盘。
- `--shared-cache` 只读取维护人员已放好的环境归档，不写共享盘。
