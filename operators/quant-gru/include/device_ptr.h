...
struct device_ptr {
    ...
    explicit device_ptr(size_t size_) : data(nullptr), size(size_) {
        void *tmp;
        cudaMalloc(&tmp, size * ElemSize);
        data = static_cast<typename T::Scalar *>(tmp);
    }

    explicit device_ptr(const T &elem) : data(nullptr), size(elem.size()) {
        void *tmp;
        cudaMalloc(&tmp, size * ElemSize);
        data = static_cast<typename T::Scalar *>(tmp);
        ToDevice(elem);
    ...
    void zero() { cudaMemset(data, 0, size * ElemSize); }

    ~device_ptr() { cudaFree(data); }
