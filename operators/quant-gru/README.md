...
**与 rx-met 分工：** 本项目提供标准 ONNX `GRU` 节点与量化参数接口；整模型导出、encodings 合并由 `src/aimet_torch/rx_export` 负责。多 GRU 模型建议导出前调用 `set_quant_gru_module_names(model)`。
