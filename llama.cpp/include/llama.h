...
extern "C" {
    ...
    struct llama_context_params {
        ...
        // TurboQuant KV-cache compression knobs (mirrors llama_cparams::tq_*).
        // Whether TQ engages for any layer is decided by .type_k / .type_v
        // above (i.e. the user's `-ctk` / `-ctv` choice); see
        // reex_turboquant::resolve_layer_spec.
        // ABI note: these fields are appended *after* all upstream fields and only
        // exist when the consumer also sees `-DREEX_TURBOQUANT`; with the macro OFF
        // the struct is binary-equivalent to upstream.
        ...
        int32_t tq_seed;            // PRNG seed; currently runtime no-op (build-time codebook)
...
#ifdef REEX_TURBOQUANT
    // === REEX_TURBOQUANT BEGIN ===
    // Force the TurboQuant InnerQ EMA per-channel scale to be finalized
    // RIGHT NOW for every layer that's still in calibration mode, even
    // if the per-layer 1024-token target was not yet reached.
    //
    // Use cases:
    //   - server / interactive flows that prefill a short prompt and want
    //     calibration to stop at ubatch boundary instead of "later when
    //     enough tokens accumulate".
    //   - benchmarking flows that want to measure post-freeze inference
    //     quality without waiting for the prefill to cross 1024 tokens.
    //   - shutdown / state-save paths that need a stable scale tensor.
    //
    // Layers with no accumulated K writes (sq_accum still all-zero)
    // freeze with identity scale (frozen-but-inactive — no Q/K rescale
    // applied, no measurable behaviour change).  Already-frozen layers
    // are no-ops.  Safe to call any number of times.
    //
    // Requires `cparams.tq_innerq_enable == true` and the K cache to be
    // tq_k3; otherwise this is a no-op and the function returns 0.
    // Returns the number of layers freshly frozen by this call.
    LLAMA_API uint32_t llama_innerq_finalize_now(struct llama_context * ctx);

    // Diagnostic queries — both return 0 when InnerQ is not engaged
    // (REEX_TURBOQUANT off at compile time, --no-tq-innerq, or K cache
    // is not tq_k3).  Iterates both base and SWA caches under iSWA and
    // sums their counts; in production typical usage one of the two
    // contains all the frozen layers.
    //
    //   - llama_innerq_frozen_layer_count: number of KV layers whose
    //     calibration has finalized (includes auto-skipped well-balanced
    //     layers).
    //   - llama_innerq_active_layer_count: number of frozen layers whose
    //     scale is non-identity (i.e. RMS imbalance >= skip_threshold,
    //     default 1.2).  When this is 0 after finalize, InnerQ has
    //     auto-skipped every layer and pays no steady-state overhead —
    //     this is the "well-behaved model, default ON is free" outcome.
    LLAMA_API uint32_t llama_innerq_frozen_layer_count(const struct llama_context * ctx);
    LLAMA_API uint32_t llama_innerq_active_layer_count(const struct llama_context * ctx);
    // === REEX_TURBOQUANT END ===
#endif
