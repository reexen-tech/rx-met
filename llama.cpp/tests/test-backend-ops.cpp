...
static void init_tensor_uniform(ggml_tensor * tensor, float min = -1.0f, float max = 1.0f) {
    ...
    } else if (tensor->type == GGML_TYPE_F64) {
        // F64 isn't a regular ggml dtype but appears as the InnerQ EMA
        // sq_accum accumulator (P4 A2.4 Stage 4).  Promote the float
        // init values to double for tensor_set; sq_accum semantics are
        // "accumulator into which atomicAdd writes", so the test
        // really only needs reproducible *initial* values across
        // backends — actual computed contents are determined by the
        // op kernel.
        std::vector<double> data_f64(nels);
        for (size_t i = 0; i < nels; ++i) {
            data_f64[i] = (double) data[i];
        }
        ggml_backend_tensor_set(tensor, data_f64.data(), 0, nels * sizeof(double));
    } else {
...
#if defined(GGML_USE_REEX) || defined(GGML_REEX_FP16_PIPELINE)
        // REEX routes quick GELU through backend-specific LUT approximations.
        // Functional validation here should tolerate approximation error across backends.
        if (op == GGML_UNARY_OP_GELU_QUICK) {
            return 3e-1;
        }
#endif
...
struct test_ssm_scan : public test_case {
    ...
    double max_nmse_err(ggml_backend_t backend) override {
        if (ggml_backend_dev_type(ggml_backend_get_device(backend)) == GGML_BACKEND_DEVICE_TYPE_GPU &&
                d_state == 16 && head_dim == 1) {
            // The CUDA d_state=16 specialized scan kernel accumulates in a different order than the CPU reference.
            return 2e-2;
        }
...
struct test_flash_attn_ext : public test_case {
    ...
    // `type_KV_v == GGML_TYPE_COUNT` means K and V share `type_KV` (legacy
    // behaviour). REEX_TURBOQUANT uses an explicit V type for mixed-K/V
    // FA-vec instances (e.g. K=F16 + V=TQ_V2). Kept as the last constructor
    // argument with a sentinel default so existing test_cases stay
    // bit-for-bit compatible.
    const ggml_type type_KV_v;

    std::string vars() override {
        std::string s = VARS_TO_STR13(hsk, hsv, nh, nr23, kv, nb, mask, sinks, max_bias, logit_softcap, prec, type_KV, permute);
        if (type_KV_v != GGML_TYPE_COUNT) {
            s += "," + VAR_TO_STR(type_KV_v);
        }
        return s;
    }
    ...
    test_flash_attn_ext(int64_t hsk = 128, int64_t hsv = 128, int64_t nh = 32, std::array<int64_t, 2> nr23 = {1, 1}, int64_t kv = 96, int64_t nb = 8,
                        ...
                        ggml_type type_KV_v = GGML_TYPE_COUNT)
        : hsk(hsk), hsv(hsv), nh(nh), nr23(nr23), kv(kv), nb(nb), mask(mask), sinks(sinks), max_bias(max_bias), logit_softcap(logit_softcap), prec(prec), type_KV(type_KV), permute(permute), type_KV_v(type_KV_v) {}

    ggml_tensor * build_graph(ggml_context * ctx) override {
        const ggml_type type_K_eff = type_KV;
        const ggml_type type_V_eff = (type_KV_v == GGML_TYPE_COUNT) ? type_KV : type_KV_v;
        const int64_t hsk_padded = GGML_PAD(hsk, ggml_blck_size(type_K_eff));
        const int64_t hsv_padded = GGML_PAD(hsv, ggml_blck_size(type_V_eff));
        ...
        ggml_tensor * k = create_permuted(type_K_eff,    hsk_padded, kv, nh,         nr23[1], true); // the K tensor is usually a view of the K cache
        ggml_set_name(k, "k");
        ...
        if (hsk_padded == 576 && hsv_padded == 512) {
            ...
            v = create_permuted(type_V_eff,  hsv_padded, kv, nh,         nr23[1], true); // the V tensor is usually a view of the V cache
        }
        ggml_set_name(v, "v");
...
//
// Test case for GGML_OP_REEX_WHT.  Builds a graph node
//   out = ggml_reex_wht(F32 input, direction, group_size, scale_inv?)
// and lets the framework compare CPU vs. (any) backend reference.  See
// tests/reex/test-reex-turboquant-backend-op-wht.cpp for the dedicated,
// richer standalone test (covers multi-thread split + bit-exact match
// against the scalar reference).
struct test_reex_wht : public test_case {
    const int64_t   dim;          // 64 or 128 (only sizes the kernel currently supports)
    const int64_t   rows;         // batch / token count along `ne[1]`
    const int       direction;    // 0 = forward, 1 = inverse
    const ggml_type scale_type;   // GGML_TYPE_COUNT means "no scale_inv"
    const bool      with_sq_accum;// P4 A2.4 Stage 4: optional InnerQ EMA F64 accumulator

    std::string vars() override {
        return VARS_TO_STR5(dim, rows, direction, scale_type, with_sq_accum);
    }

    test_reex_wht(int64_t dim_ = 128, int64_t rows_ = 4,
                  int direction_ = 0, ggml_type scale_type_ = GGML_TYPE_COUNT,
                  bool with_sq_accum_ = false)
        : dim(dim_), rows(rows_), direction(direction_), scale_type(scale_type_),
          with_sq_accum(with_sq_accum_) {}

    ggml_tensor * build_graph(ggml_context * ctx) override {
        ggml_tensor * x = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, dim, rows);
        ggml_set_param(x);
        ggml_set_name(x, "x");

        ggml_tensor * scale = nullptr;
        if (scale_type != GGML_TYPE_COUNT) {
            scale = ggml_new_tensor_1d(ctx, scale_type, dim);
            ggml_set_name(scale, "scale_inv");
        }

        // P4 A2.4 Stage 4: when with_sq_accum is true, allocate an F64
        // accumulator tensor and pass it as src[2].  The op writes into
        // it via atomicAdd on CUDA / serial += on CPU.  This test_case
        // only verifies that the rotated `y` output is bit-identical
        // CPU vs CUDA *while sq_accum is being filled in parallel* —
        // the actual cross-backend numerical parity of the sq_accum
        // *values* is guarded separately by
        // tests/reex/test-reex-turboquant-backend-op-wht.cpp's
        // run_sq_accum_case (CPU graph) and by e2e perplexity parity
        // (Stage 5: PPL with CUDA-native sq_accum bit-identical to the
        // earlier CPU-offload result, see Stage 4 README).
        ggml_tensor * sq_accum = nullptr;
        if (with_sq_accum) {
            sq_accum = ggml_new_tensor_1d(ctx, GGML_TYPE_F64, dim);
            ggml_set_name(sq_accum, "sq_accum");
        }

        ggml_tensor * out = ggml_reex_wht(ctx, x, direction, (int) dim, scale, sq_accum);
        ggml_set_name(out, "out");
        return out;
    }
};
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // REEX FP16 pipeline: rms_norm with F16 input
    for (uint32_t n : { 64, 1025 }) {
        test_cases.emplace_back(new test_rms_norm(GGML_TYPE_F16, { n, 5, 4, 3 }, false, 1e-6f));
    }
#endif
...
#ifdef GGML_REEX_FP16_PIPELINE
    // REEX FP16 pipeline: mul_mat with quantized weight x F16 activation
    // k must be >= quant block size (32 for Q4_0, 256 for Q4_K)
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q4_0, GGML_TYPE_F16, 32,  32, 32,   {1, 1}, {1, 1}));
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q4_0, GGML_TYPE_F16, 1,   64, 256,  {1, 1}, {1, 1}));
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q4_K, GGML_TYPE_F16, 32,  32, 256,  {1, 1}, {1, 1}));
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_F16,  GGML_TYPE_F16, 64,  2,  64,   {1, 1}, {1, 1}));
    // REEX FP16 pipeline: mul_mat_id with quantized weight x F16 activation
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_Q4_0, GGML_TYPE_F16, 8, 2, false, 32, 32, 32));
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_Q4_K, GGML_TYPE_F16, 8, 2, false, 32, 32, 256));
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_F16,  GGML_TYPE_F16, 8, 2, false, 32, 32, 32));
#endif
    ...
    // >>> REEX (reexen): block-64 test cases for Q4_0_64 / Q4_K_64 / block-64 K-quants.
    //
    // Why this replaces the old block (deleted): the previous block was guarded by
    //   #ifdef GGML_USE_REEX_Q64
    //   ... Q4_0_64 / Q4_K_64 MUL_MAT_ID cases ...
    //   #endif
    // That compile-time guard never fired in this translation unit: ggml-base marks
    // GGML_USE_REEX_Q64 as PRIVATE (dllexport/dllimport linkage), so the macro is
    // not visible at compile time to test-backend-ops. The #ifdef therefore always
    // evaluated to false and those MUL_MAT_ID cases were dead code — never compiled
    // in, never run. Keeping the #ifdef block would silently keep them disabled.
    //
    // New implementation: replace the compile-time #ifdef with a RUNTIME guard —
    // skip a type when ggml_get_type_traits(type)->to_float == nullptr, i.e. the
    // linked ggml was built without GGML_USE_REEX_Q64 and did not register these
    // types. The enum values always exist in ggml.h regardless of the build, so the
    // cases can always be instantiated; they no-op cleanly when the type is absent.
    // No extra compile definition is needed for the test binary.
    //
    // Coverage added on top of the deleted block (was MUL_MAT_ID only):
    //   - MUL_MAT (single/batched weights) + GET_ROWS for Q4_0_64 / Q4_K_64,
    //   - MUL_MAT + batched + MUL_MAT_ID + GET_ROWS for the symmetric K-quants.
    // k = 256 covers Q4_0_64 (4×64) and one Q4_K_64 super-block; n > 8 exercises the
    // CUDA mmvq prefill token-chunking; batched ne2/ne3 cover per-expert offsets.
    // <<< REEX
    for (ggml_type type_a : {GGML_TYPE_Q4_0_64, GGML_TYPE_Q4_K_64}) {
        if (ggml_get_type_traits(type_a)->to_float == nullptr) {
            continue; // REEX: ggml built without GGML_USE_REEX_Q64 — type not registered
        }
    ...
    // >>> REEX (reexen): block-64 symmetric K-quants — same four CUDA weight->F32
    // paths (dequant, mmvq, mmq, get_rows) plus MoE MUL_MAT_ID, runtime-guarded as
    // above. MMQ only covers Q6_K_64/Q3_K_64; the S types fall through the dequant
    // path.
    // <<< REEX
...
#ifdef GGML_REEX_FP16_PIPELINE
    // REEX FP16 pipeline: soft_max with F16 src0
    test_cases.emplace_back(new test_soft_max(GGML_TYPE_F16, {16,   16,   1, 1}, false, false, GGML_TYPE_F32, {1, 1}, 1.0f, 0.0f));
    test_cases.emplace_back(new test_soft_max(GGML_TYPE_F16, {1024, 16,   1, 1}, false, false, GGML_TYPE_F32, {1, 1}, 0.1f, 0.0f));
    test_cases.emplace_back(new test_soft_max(GGML_TYPE_F16, {16,   1024, 1, 1}, true,  false, GGML_TYPE_F16, {1, 1}, 1.0f, 0.0f));
    test_cases.emplace_back(new test_soft_max(GGML_TYPE_F16, {1024, 1024, 1, 1}, true,  false, GGML_TYPE_F32, {1, 1}, 0.1f, 8.0f));
#endif
...
#ifdef REEX_TURBOQUANT
    // === REEX_TURBOQUANT BEGIN ===
    // FA-vec for TurboQuant K/V combinations. Instances ship in
    // `template-instances/fattn-vec-instance-{f16,tq_k3}-{tq_v2,tq_v4}.cu`;
    // this exercises the full path
    // `flash_attn_ext_supported → get_best_fattn_kernel → flash_attn_ext_vec
    // → ggml_cuda_flash_attn_ext_vec_case_impl<D, K, V> → vec_dot_KQ + dequantize_V`.
    // hd=128 (Llama-3 / Qwen2-7B) and hd=256 (Qwen3 series); both are valid
    // because `vec_dot_fattn_vec_KQ_tq_k3` walks `D / QK_TQ_K3` blocks.
    // mask=true / sinks=false / max_bias=0 / logit_softcap=0 are required by
    // the FA-vec contract for these K/V combos (see fattn.cu's
    // ggml_cuda_get_best_fattn_kernel).
    for (int64_t hd : { 128, 256 }) {
        for (int64_t kv : { 256, 1024 }) {
            // K=F16 + V=TQ_V2 (V-only quant; covers the same D ladder upstream
            // ships for vanilla quants).
            test_cases.emplace_back(new test_flash_attn_ext(
                hd, hd, 4, {1, 1}, kv, 1, true, false, 0.0f, 0.0f, GGML_PREC_F32,
                GGML_TYPE_F16, {0, 1, 2, 3}, GGML_TYPE_TQ_V2));
            // K=TQ_K3 + V=TQ_V2 (the headline configuration).
            test_cases.emplace_back(new test_flash_attn_ext(
                hd, hd, 4, {1, 1}, kv, 1, true, false, 0.0f, 0.0f, GGML_PREC_F32,
                GGML_TYPE_TQ_K3, {0, 1, 2, 3}, GGML_TYPE_TQ_V2));
            // K=TQ_K3 + V=TQ_V4 (high-precision V variant).
            test_cases.emplace_back(new test_flash_attn_ext(
                hd, hd, 4, {1, 1}, kv, 1, true, false, 0.0f, 0.0f, GGML_PREC_F32,
                GGML_TYPE_TQ_K3, {0, 1, 2, 3}, GGML_TYPE_TQ_V4));
            // P4 A2.5 V-β: K=TQ_K3 + V=TQ_V_POLAR2 (Lloyd-Max 2-bit, TheTom turbo2_0).
            test_cases.emplace_back(new test_flash_attn_ext(
                hd, hd, 4, {1, 1}, kv, 1, true, false, 0.0f, 0.0f, GGML_PREC_F32,
                GGML_TYPE_TQ_K3, {0, 1, 2, 3}, GGML_TYPE_TQ_V_POLAR2));
            // P4 A2.5 V-β: K=TQ_K3 + V=TQ_V_POLAR4 (Lloyd-Max 4-bit, TheTom turbo4_0).
            test_cases.emplace_back(new test_flash_attn_ext(
                hd, hd, 4, {1, 1}, kv, 1, true, false, 0.0f, 0.0f, GGML_PREC_F32,
                GGML_TYPE_TQ_K3, {0, 1, 2, 3}, GGML_TYPE_TQ_V_POLAR4));
        }
    }
    // === REEX_TURBOQUANT END ===
#endif
...
#ifdef REEX_TURBOQUANT
    // === REEX_TURBOQUANT BEGIN ===
    // GGML_OP_REEX_WHT: forward / inverse, with and without optional scale_inv.
    // Sweep (dim, rows, direction, scale dtype).
    for (int direction : {0, 1}) {
        for (int64_t dim : {64, 128}) {
            for (int64_t rows : {1, 4, 17}) {
                test_cases.emplace_back(new test_reex_wht(dim, rows, direction, GGML_TYPE_COUNT));
                test_cases.emplace_back(new test_reex_wht(dim, rows, direction, GGML_TYPE_F32));
                test_cases.emplace_back(new test_reex_wht(dim, rows, direction, GGML_TYPE_F16));
                // P4 A2.4 Stage 4: sq_accum CUDA parity.  Forward direction
                // only because that's what InnerQ EMA uses on the K cpy
                // path; pair with no-scale and F32-scale variants — F16
                // scale is uncovered because build_attn never emits F16
                // scale + sq_accum together.
                test_cases.emplace_back(new test_reex_wht(dim, rows, direction, GGML_TYPE_COUNT, /*with_sq_accum=*/true));
                test_cases.emplace_back(new test_reex_wht(dim, rows, direction, GGML_TYPE_F32,   /*with_sq_accum=*/true));
            }
        }
    }

    // GGML_TYPE_TQ_K3 / TQ_V2 / TQ_V4: exercise the type_traits round-trip
    // through CPY (F32 -> TQ -> F32) and GET_ROWS (TQ -> F32) inside the
    // standard test-backend-ops framework.  Block size constraints:
    //   TQ_K3 -> ne[0] must be a multiple of 128
    //   TQ_V2 -> ne[0] must be a multiple of 32
    //   TQ_V4 -> ne[0] must be a multiple of 32
    {
        struct tq_entry { ggml_type type; int64_t blk; };
        const tq_entry tq_types[] = {
            { GGML_TYPE_TQ_K3,       128 },
            { GGML_TYPE_TQ_V2,        32 },
            { GGML_TYPE_TQ_V4,        32 },
            // P4 A2.5 V-β: Lloyd-Max + norm correction (TheTom turbo2_0 / turbo4_0).
            { GGML_TYPE_TQ_V_POLAR2, 128 },
            { GGML_TYPE_TQ_V_POLAR4, 128 },
        };
        for (const auto & e : tq_types) {
            for (int k = 1; k <= 2; ++k) {
                test_cases.emplace_back(new test_cpy(GGML_TYPE_F32, e.type, {k * e.blk, 4, 4, 4}));
                test_cases.emplace_back(new test_cpy(e.type, GGML_TYPE_F32, {k * e.blk, 4, 4, 4}));
            }
            test_cases.emplace_back(new test_get_rows(e.type, e.blk, 5, 4, 1, 1, false));
            test_cases.emplace_back(new test_get_rows(e.type, e.blk, 5, 4, 2, 2, false));

            // SET_ROWS: F32 -> TQ; index tensor I32 / I64. Exercises the cpy-utils
            // quantize_f32_<type>_block path on the destination side. Mirrors the
            // KV-cache write performed by llama_kv_cache::cpy_k / cpy_v on CUDA.
            for (int b : {1, 7}) {
                for (bool v : {false, true}) {
                    test_cases.emplace_back(new test_set_rows(e.type, GGML_TYPE_I64, { e.blk, 5, b, 3 }, { 1, 1 }, 1, v));
                    test_cases.emplace_back(new test_set_rows(e.type, GGML_TYPE_I64, { 2 * e.blk, 11, 1, b }, { 2, 3 }, 7, v));
                }
            }

            // MUL_MAT: A is TQ-quantized [k, m], B is F32 [k, n] (vec_dot_type=F32),
            // result is F32 [m, n].  Pick small representative shapes that exercise
            // the dequantize-then-dot kernel without blowing up runtime.
            test_cases.emplace_back(new test_mul_mat(e.type, GGML_TYPE_F32, /*m=*/4,  /*n=*/1, /*k=*/e.blk, {1, 1}, {1, 1}));
            test_cases.emplace_back(new test_mul_mat(e.type, GGML_TYPE_F32, /*m=*/16, /*n=*/4, /*k=*/2 * e.blk, {1, 1}, {1, 1}));
        }
    }
    // === REEX_TURBOQUANT END ===
#endif
