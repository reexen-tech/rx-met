...
int main(int argc, char ** argv) {
    ...
    if (params.hellaswag || params.winogrande || params.multiple_choice) {
        ...
        // *REEXEN change*
        // multi-sequence batch has "coupled" sequences (shared prefix + branches);
        // split_equal(sequential) cannot handle that and returns empty -> decode fails.
        // unified KV cache uses split_simple instead, so enable it for these modes.
