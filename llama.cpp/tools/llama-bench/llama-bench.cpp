...
static ggml_type ggml_type_from_name(const std::string & s) {
    ...
    // P3.6: surface TQ K/V types so quality_gate / llama-bench can run them
    // as a peer of q8_0 / f16. The string keys match `ggml_type_name`
    // (ggml.c type_traits): tq_k3 (3-bit K) / tq_v2 (2-bit V) / tq_v4 (4-bit V).
    // FA-vec K/V allow-list is the same as everywhere else (fattn.cu's
    // get_best_fattn_kernel decides; mismatched K/V is rejected at runtime).
    if (s == "tq_k3") { return GGML_TYPE_TQ_K3; }
    if (s == "tq_v2") { return GGML_TYPE_TQ_V2; }
    if (s == "tq_v4") { return GGML_TYPE_TQ_V4; }
    // === REEX_TURBOQUANT END ===
#endif
