        ...
        for (int64_t m = 0; m < M; ++m)
            for (int64_t n = 0; n < N; ++n) {
                ...
                for (int64_t sb = 0; sb < sb_per_row; ++sb) {
                    ...
                    acc += (double) rgd_q6k64_dot_superblock(w, a_blocks.data(), m, sb, M, K, ts, psum_bits);
                }
        ...
        for (int64_t m = 0; m < M; ++m)
            for (int64_t n = 0; n < N; ++n) {
                ...
                for (int64_t sb = 0; sb < sb_per_row; ++sb) {
                    ...
                    acc += (double) rgd_q5k64s_dot_superblock(w, a_blocks.data(), m, sb, M, K, ts, psum_bits);
                }
                C_ref_tiled[result_tiled_index(m, n, M, ts)] = (float) acc;
            }
    } else { // Legacy q8_0_64
        ...
        for (int64_t m = 0; m < M; ++m)
            for (int64_t n = 0; n < N; ++n) {
                ...
                for (int64_t kbw = 0; kbw < kb_per_row; ++kbw) {
                    ...
                    acc += (double) rgd_q8_0_64_dot_block(w, a_blocks.data(), m, kbw, M, K, ts, psum_bits);
                }
                C_ref_tiled[result_tiled_index(m, n, M, ts)] = (float) acc;
            }
    }
...
DequantCheck golden_dequant_check(int wtype_id,
    ...
    const int     stride   = act_block_bytes(ts);
    ...
    for (int64_t m = 0; m < rows; ++m) {
        for (int64_t kg = 0; kg < kg_per_row; ++kg) {
            ...
            for (int j = 0; j < agroup; ++j)
                Arow[(size_t) (kg * agroup + j)] = ad * (float) qs[j];
        }
        for (int64_t n = 0; n < N; ++n) {
            double acc = 0.0;
