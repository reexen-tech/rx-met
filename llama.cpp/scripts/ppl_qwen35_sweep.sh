...
run_one_type() {
  local type="$1"
  local model="${2:-${OUT}/${NAME}-${type}.gguf}"
  local ppl_log="${OUT}/ppl_${type}.log"
  local blocked="${OUT}/${type}.ppl.blocked"

  rm -f "${blocked}"

  if [[ ! -f "${model}" ]]; then
    echo "[BLOCKED] missing model for ${type}: ${model}" | tee "${blocked}"
    return 0
  fi
...
case "${PHASE}" in
  one)
    type="${2:?Usage: $0 one <TYPE> <GGUF_PATH>}"
    model="${3:?Usage: $0 one <TYPE> <GGUF_PATH>}"
    run_base_ppl
    run_one_type "${type}" "${model}"
    ;;
  legacy0)
  ...
  status)
    ;;
  *)
    echo "Usage: $0 {legacy0|k|iq|all|status|one <TYPE> <GGUF_PATH>}" >&2
    exit 2
    ;;
esac
