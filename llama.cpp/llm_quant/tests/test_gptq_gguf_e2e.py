...
from llm_quant.gptq.sidecar import write_sidecar_v2
from validate_gptq_gguf import validate_gguf, validate_sidecar

import convert_hf_to_gguf as converter
import gguf

def _write_tiny_qwen2(path: Path, *, sidecar_version: int = 1) -> None:
    ...
    for name, module in model.named_modules():
        if not isinstance(module, torch.nn.Linear) or not name.startswith("model.layers."):
            continue
        result = quantize_q4_0_64(module.weight.detach())
    ...
    if sidecar_version == 1:
        torch.save(
            {"format": "Q4_0_64", "version": 1, "tensors": tensors},
            path / "gptq_q4_0_64.pt",
        )
    else:
        write_sidecar_v2(path, tensors, source_model="tiny-qwen2")


def test_sidecar_is_written_directly_to_gguf(tmp_path: Path, monkeypatch) -> None:
    model_path = tmp_path / "tiny-qwen2"
    model_path.mkdir()
    _write_tiny_qwen2(model_path)
    ...
    model.set_vocab()
    model.write()
    tensors = validate_sidecar(model_path / "gptq_q4_0_64.pt")
    validate_gguf(tensors, gguf_path)


def test_packed_only_sidecar_v2_is_written_directly_to_gguf(
