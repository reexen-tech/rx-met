...
struct llm_graph_context {
    ...
    // [REEX] Per-model FP16 pipeline enable flag (NPU-equivalent data flow).
    // Default false = upstream behavior. Only set to true by models that have been
    // validated for FP16 pipeline. This is the single source of truth for all
    // REEX pipeline decisions within this graph context.
    bool reex_fp16_pipeline = false;
    ...
    // [REEX] FP16 pipeline cast helpers.
    // No-op when reex_fp16_pipeline == false or tensor is already the target type.
    // Used at model-level data flow boundaries to simulate NPU 16-bit data bus.
    ggml_tensor * reex_cast_f16(ggml_tensor * t) const;
    ggml_tensor * reex_cast_f32(ggml_tensor * t) const;
    ...
    // [REEX] FP16 pipeline casts inside are controlled by this->reex_fp16_pipeline.
    // When reex_fp16_pipeline == true, inserts FP16 casts at GEMM→activation boundaries
    // to simulate NPU 16-bit data bus.
    ggml_tensor * build_moe_ffn(
             ggml_tensor * cur,
             ggml_tensor * gate_inp,
    ...
    ggml_tensor * build_attn_mha(
            ...
            ggml_tensor * v_mla,   // [n_embd_head_v_mla, n_embd_head_v, n_head_v]
                  float   kq_scale,
                    int   il,
            // [REEX_TURBOQUANT] P4 A2.4 Stage 2b-c: pre-WHT scale_inv for the
            // Q-side `Q ⊙ scale_inv` rescale that complements the K-side
            // `K ⊙ scale` written into the cache by cpy_k.  Identity-only
            // until InnerQ has frozen this layer; null otherwise (skipped
            // by ggml_reex_wht's no-scale fast path).  Callers without a
            // KV cache (no_cache / cross attention) leave this nullptr.
            ggml_tensor * tq_q_scale_inv = nullptr) const;
