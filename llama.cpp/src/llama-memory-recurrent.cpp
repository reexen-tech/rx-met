...
llama_memory_recurrent::llama_memory_recurrent(
    ...
    const layer_filter_cb & filter) : hparams(model.hparams), n_seq_max(n_seq_max), reex_custom_unified_split(reex_custom_unified_split) {
    ...
    do {
        ...
        while (true) {
            ...
            if (embd_all) {
                ...
                // [REEX custom] Keep this local multi-seq split policy behind an
                // independent switch instead of the upstream kv_unified flag.
                ubatch = balloc.split_equal(n_ubatch, !reex_custom_unified_split);
            }
