...
public:
    llama_kv_cache_iswa(
        ...
        // [REEX_TURBOQUANT] forwarded to both base/swa kv_caches
...
public:
    ...
    // P4 A2.4 Stage 2b — propagate lctx to both wrapped contexts so each
    // can sched_synchronize() in apply().  Called from
    // llama_context::process_ubatch via dynamic_cast.
