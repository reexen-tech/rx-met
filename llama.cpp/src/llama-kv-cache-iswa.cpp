...
llama_kv_cache_iswa::llama_kv_cache_iswa(
    ...
    const llama_cparams *   cparams) : hparams(model.hparams), unified(unified) {
    ...
    kv_base = std::make_unique<llama_kv_cache>(
            ...
            0, LLAMA_SWA_TYPE_NONE, filter_base, reuse, cparams);
    ...
    kv_swa = std::make_unique<llama_kv_cache>(
            ...
            hparams.n_swa, hparams.swa_type, filter_swa, reuse, cparams);
}
...
void llama_kv_cache_iswa_context::tq_innerq_set_lctx(llama_context * lctx_in) {
    if (auto * b = dynamic_cast<llama_kv_cache_context*>(ctx_base.get())) {
        b->tq_innerq_set_lctx(lctx_in);
    }
    if (auto * s = dynamic_cast<llama_kv_cache_context*>(ctx_swa.get())) {
        s->tq_innerq_set_lctx(lctx_in);
    }
}
#endif
