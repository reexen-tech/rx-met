...
#include "llama-kv-cache-iswa.h"  // for dynamic_cast in tq_innerq_set_lctx
#endif
        ...
        const llama_model & model,
                ...
                ggml_type   type_s,
                 ...
                 uint32_t   n_seq_max,
                     bool   offload,
                    bool   unified,
                    bool   reex_custom_unified_split,
                            /* layer filters */
    ...
    do {
        ...
        while (true) {
            ...
            if (embd_all) {
                ...
                // [REEX custom] Keep the local split override while also honoring the
                // upstream unified-KV detection.
                const bool unified = (mem_attn->get_base()->get_n_stream() == 1) || reex_custom_unified_split;
...
void llama_memory_hybrid_iswa_context::tq_innerq_set_lctx(llama_context * lctx_in) {
    if (auto * a_iswa = dynamic_cast<llama_kv_cache_iswa_context*>(ctx_attn.get())) {
        a_iswa->tq_innerq_set_lctx(lctx_in);
    } else if (auto * a = dynamic_cast<llama_kv_cache_context*>(ctx_attn.get())) {
        a->tq_innerq_set_lctx(lctx_in);
    }
}
// === REEX_TURBOQUANT END ===
#endif
