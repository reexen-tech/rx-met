...
llama_memory_i * llama_model::create_memory(const llama_memory_params & params, const llama_cparams & cparams) const {
    llama_memory_i * res;
    const char * reex_custom_split_env = std::getenv("REEX_CUSTOM_UNIFIED_SPLIT");
    const auto reex_custom_split_flag = llama_reex_parse_env_flag(reex_custom_split_env);

    if (reex_custom_split_flag == llama_reex_env_flag::invalid) {
        LLAMA_LOG_WARN("%s: ignoring invalid REEX_CUSTOM_UNIFIED_SPLIT=%s (expected one of: 0/1, false/true, off/on, no/yes)\n",
                __func__, reex_custom_split_env);
    }

    const bool reex_custom_unified_split =
        llama_reex_should_use_custom_unified_split(cparams.kv_unified, reex_custom_split_flag);

    if (reex_custom_unified_split) {
        LLAMA_LOG_INFO("%s: enabling REEX_CUSTOM_UNIFIED_SPLIT=%s for recurrent/hybrid multi-sequence splitting\n",
                __func__, reex_custom_split_env);
    }
                ...
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
                        ...
                        res = new llama_memory_hybrid_iswa(
                            ...
                            /* reex_custom_unified_split */ reex_custom_unified_split,
