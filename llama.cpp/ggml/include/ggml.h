...
extern "C" {
    ...
    enum ggml_type {
        ...
        // TurboQuant KV-cache compression types. Layout fixed in
        // docs/turboquant/02_data_layout.md and ggml/src/ggml-common.h.
        //
        // Private-fork warning:
        // These ids are currently used only for runtime KV-cache quantization,
        // not for persisted GGUF tensors. They may be renumbered while that
        // remains true. Once any GGUF tensor is written with a GGML_TYPE_TQ_*
        // id, the number becomes part of the file format contract for this
        // fork and must not be moved to follow upstream. If upstream later
        // claims the same numeric id, handle it with an explicit migration or
        // loader rejection; never silently reinterpret old GGUF files.
        GGML_TYPE_TQ_K3       = 42, // 3-bit polar-quant K (rotated via K WHT; QJL residual signs; block=128)
        GGML_TYPE_TQ_V2       = 43, // 2-bit V (rotated via V WHT; scale+zero codebook; block=32; legacy double-track)
        GGML_TYPE_TQ_V4       = 44, // 4-bit V (rotated via V WHT; scale+zero codebook; block=32; legacy double-track)
    ...
    enum ggml_op {
        ...
        // === REEX_TURBOQUANT BEGIN ===
        GGML_OP_REEX_WHT, // Walsh-Hadamard rotation; see ggml_reex_wht
        // === REEX_TURBOQUANT END ===
        GGML_OP_COUNT,
    };
    ...
    // === REEX_TURBOQUANT BEGIN ===
    // Walsh-Hadamard rotation used to scramble keys / queries before
    // flash-attention.  `a` is treated as flat rows of length `group_size`;
    // each row is rotated independently in-place by an in-place
    // sign1 · butterfly · sign2 WHT.  Sign tables come from
    // ggml/src/ggml-cpu/reex/reex_turboquant_wht_signs.inc (or the
    // backend-specific equivalent).
    //
    // direction  : 0 = forward,  1 = inverse
    // group_size : 0 → use a->ne[0]; otherwise must be a power of two that
    //              divides a->ne[0] (only 64 and 128 are wired in Phase 1).
    // scale_inv  : optional contiguous f16/f32 1-D tensor of length
    //              `group_size`.  When non-NULL, it is applied element-wise
    //              to the input row BEFORE the rotation, implementing
    //              TurboQuant InnerQ EMA's per-channel rescaling.  The
    //              pre-rotation placement is required by the inner-product
    //              invariant `<Q ⊙ s_inv, K ⊙ s> = <Q, K>` — H_d is
    //              orthogonal but does NOT commute with diagonal scaling.
    // sq_accum   : optional contiguous f64 1-D tensor of length `group_size`
    //              used as a per-channel post-WHT variance accumulator
    //              during InnerQ EMA's calibration window.  When non-NULL
    //              the kernel adds `out[i] * out[i]` into `sq_accum[i]`
    //              (post-normalisation) for every row processed.  The CPU
    //              dispatcher pins this op to a single thread when
    //              sq_accum != NULL to keep the accumulator update serial.
    //              Not currently supported on the CUDA backend; the CUDA
    //              `supports_op` predicate returns false when sq_accum is
    //              non-NULL so the scheduler will offload the op to CPU.
    //              `scale_inv` and `sq_accum` are independent and either,
    //              both, or neither may be non-NULL.
    GGML_API struct ggml_tensor * ggml_reex_wht(
            struct ggml_context * ctx,
            struct ggml_tensor  * a,
            int                   direction,
            int                   group_size,
