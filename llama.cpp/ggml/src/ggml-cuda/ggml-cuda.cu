...
#include "ggml-cuda/reex/reex_cuda_trace.cuh"
#if defined(GGML_USE_REEX_GEMM_CUDA)
#include "ggml-cuda/reex/reex_dispatch.cuh"
#endif

#ifdef REEX_TURBOQUANT
#include "ggml-cuda/reex/reex_turboquant_wht.cuh"
#endif
...
static __global__ void kernel_f16_to_f32(const half * __restrict__ src, float * __restrict__ dst, const int64_t n) {
...
static ggml_tensor ggml_cuda_cast_src1_f16_to_f32_preserve_layout(
        ggml_backend_cuda_context & ctx,
        const ggml_tensor * src,
        ggml_cuda_pool_alloc<float> & storage) {
    GGML_ASSERT(src->type == GGML_TYPE_F16);

    size_t span_bytes = ggml_type_size(src->type);
    for (int i = 0; i < GGML_MAX_DIMS; ++i) {
        GGML_ASSERT(src->nb[i] % sizeof(half) == 0);
        if (src->ne[i] > 0) {
            span_bytes += (src->ne[i] - 1) * src->nb[i];
        }
    }

    const int64_t raw_f16_elems = (span_bytes + sizeof(half) - 1) / sizeof(half);
    storage.alloc(ctx.pool(), raw_f16_elems);
    kernel_f16_to_f32<<<(raw_f16_elems + 255)/256, 256, 0, ctx.stream()>>>(
        (const half *) src->data, storage.get(), raw_f16_elems);

    ggml_tensor dst = *src;
    dst.type = GGML_TYPE_F32;
    dst.data = storage.get();
    for (int i = 0; i < GGML_MAX_DIMS; ++i) {
        dst.nb[i] = (src->nb[i] / sizeof(half)) * sizeof(float);
    }
    return dst;
}
#endif // GGML_REEX_FP16_PIPELINE

static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
#ifdef GGML_REEX_FP16_PIPELINE
    ggml_tensor src1_f32_storage;
    ggml_cuda_pool_alloc<float> src1_f32_alloc;
    if (src1->type == GGML_TYPE_F16) {
        src1_f32_storage = ggml_cuda_cast_src1_f16_to_f32_preserve_layout(ctx, src1, src1_f32_alloc);
        src1 = &src1_f32_storage;
    }
#endif // GGML_REEX_FP16_PIPELINE
    ...
    // TQ_K3 / TQ_V2 / TQ_V4 / TQ_V_POLAR2 / TQ_V_POLAR4 do not have MMQ /
    // MMVQ vec_dot kernels yet (Phase 3.4 ships dequant-then-fp32 only).
    // Steer them to the cublas fallback below — `ggml_cuda_op_mul_mat_cublas`
    // already handles `is_quantized(src0)` by dequantising via
    // `ggml_get_to_fp16_cuda`, which we extended in Phase 3.1.
    if (src0->type == GGML_TYPE_TQ_K3       ||
        src0->type == GGML_TYPE_TQ_V2       ||
        src0->type == GGML_TYPE_TQ_V4       ||
        src0->type == GGML_TYPE_TQ_V_POLAR2 ||
        src0->type == GGML_TYPE_TQ_V_POLAR4) {
    ...
    // REEX W4×INT16: MUL_MAT small-batch MMVQ (M <= 8).
    // MUL_MAT_ID is dispatched separately in ggml_cuda_mul_mat_id().
    if (ggml_reex_cuda_should_use_q16_mul_mat(src0, src1, dst, split, bad_padding_clear)) {
        ggml_cuda_mul_mat_reex_q16(ctx, src0, src1, nullptr, dst);
    } else
#endif
    if (!split && use_mul_mat_vec_f) {
        ...
        // REEX W4×INT8 when src0 is Q4_K: F32 → Q8_1 quantize + Q4_K×Q8_1 vec_dot (quantize_row_q8_1_cuda below)
        ggml_reex_cuda_note_q8_mul_mat_dispatch(src0);
...
#ifdef GGML_REEX_FP16_PIPELINE
    ggml_tensor src1_f32_id_storage;
    ggml_cuda_pool_alloc<float> src1_f32_id_alloc;
    if (src1->type == GGML_TYPE_F16) {
        src1_f32_id_storage = ggml_cuda_cast_src1_f16_to_f32_preserve_layout(ctx, src1, src1_f32_id_alloc);
        src1 = &src1_f32_id_storage;
    }
#endif // GGML_REEX_FP16_PIPELINE
...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->op == GGML_OP_MUL_MAT_ID) {
            ...
            bool disable_cuda_graph_for_mmid = !ggml_is_quantized(node->src[0]->type) || node->ne[2] > mmvq_mmid_max;
#if defined(GGML_USE_REEX_GEMM_CUDA) && defined(GGML_REEX_GEMM_ACTIVATION_Q16)
            if (ggml_reex_cuda_is_q16_mul_mat_id_graph_candidate(node)) {
                disable_cuda_graph_for_mmid = ggml_reex_cuda_should_disable_cuda_graph_for_q16_mul_mat_id(node);
            }
#endif
            if (disable_cuda_graph_for_mmid) {
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
...
#ifndef GGML_REEX_FP16_PIPELINE
...
#ifdef REEX_TURBOQUANT
                    // === REEX_TURBOQUANT BEGIN ===
                    // Phase 3.4: TQ_* MUL_MAT goes through the cublas dequant
                    // fallback in ggml_cuda_mul_mat (no MMQ / MMVQ kernels).
                    // Only B == F32 and dst == F32 are supported because the
                    // dequant path produces F16/F32 src0 and we don't touch
                    // src1 routing.
                    case GGML_TYPE_TQ_K3:
                    case GGML_TYPE_TQ_V2:
                    case GGML_TYPE_TQ_V4:
...
#ifdef REEX_TURBOQUANT
                    || op->type == GGML_TYPE_TQ_K3       || op->type == GGML_TYPE_TQ_V2 || op->type == GGML_TYPE_TQ_V4
                    || op->type == GGML_TYPE_TQ_V_POLAR2 || op->type == GGML_TYPE_TQ_V_POLAR4
#endif
                    ;
                return dst_ok &&
...
#ifdef REEX_TURBOQUANT
                if (src0_type == GGML_TYPE_F32 && (src1_type == GGML_TYPE_TQ_K3       ||
                                                   src1_type == GGML_TYPE_TQ_V2       ||
                                                   src1_type == GGML_TYPE_TQ_V4       ||
                                                   src1_type == GGML_TYPE_TQ_V_POLAR2 ||
                                                   src1_type == GGML_TYPE_TQ_V_POLAR4)) {
                    return true;
                }
                if ((src0_type == GGML_TYPE_TQ_K3       || src0_type == GGML_TYPE_TQ_V2 || src0_type == GGML_TYPE_TQ_V4
                  || src0_type == GGML_TYPE_TQ_V_POLAR2 || src0_type == GGML_TYPE_TQ_V_POLAR4)
        ...
        case GGML_OP_REEX_WHT: {
            ...
            if (src0 == nullptr || src0->type != GGML_TYPE_F32 || op->type != GGML_TYPE_F32) {
                return false;
            }
            const int32_t group_size = op->op_params[1];
            if (group_size != 64 && group_size != 128) {
                return false;
            }
            if (src0->ne[0] % group_size != 0) {
                return false;
            }
            // Phase 3.3: only F32 scale_inv supported on CUDA. F16 stays on CPU
            // (the TQ build_attn path emits F32 scale tensors today; F16 is only
            // exercised by tests that already pin -b CPU).
            if (src1 != nullptr && src1->type != GGML_TYPE_F32) {
                return false;
            }
            // P4 A2.4 Stage 4: InnerQ EMA's per-channel sq_accum calibration
            // accumulator is now natively supported via atomicAdd(double*,
            // double) (sm_60+).  Validate that src[2] is the right shape +
            // type (F64, ne[0] == group_size) when present so the scheduler
            // can confidently dispatch to CUDA.  has_sq_accum lives in
            // op_params[3] (see ggml_reex_wht).
            const int32_t has_sq_accum = op->op_params[3];
            if (has_sq_accum) {
                const ggml_tensor * src2 = op->src[2];
                if (src2 == nullptr || src2->type != GGML_TYPE_F64) {
                    return false;
                }
                if (src2->ne[0] != group_size) {
                    return false;
                }
            }
            return true;
