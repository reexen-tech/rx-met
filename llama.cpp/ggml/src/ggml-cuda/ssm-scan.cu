...
#pragma unroll
    for (size_t i = 0; i < L; i++)
    {
        ...
        {
#ifdef GGML_USE_REEX
            dt_soft_plus = ggml_cuda_log_lut_mixed_fp16_reex(1.0f + ggml_cuda_exp_lut_mixed_fp16_reex(dt_soft_plus));
#else
...
#pragma unroll
        ...
        {
#ifdef GGML_USE_REEX
            float state = regs0[n] * ggml_cuda_exp_lut_mixed_fp16_reex(dt_soft_plus * regA[n]) + smemB[n] * x_dt;
#else
...
#pragma unroll
    ...
    for (int64_t i = 0; i < n_tok; i++) {
...
#ifdef GGML_USE_REEX
        const float dt_soft_plus = (dt_warp[i * stride_dt] <= 20.0f ? ggml_cuda_log_lut_mixed_fp16_reex(1.0f + ggml_cuda_exp_lut_mixed_fp16_reex(dt_warp[i * stride_dt])) : dt_warp[i * stride_dt]);
#else
        ...
        state_sum = 0.0f;
#ifdef GGML_USE_REEX
        const float dA   = ggml_cuda_exp_lut_mixed_fp16_reex(dt_soft_plus * A_warp[0]);
#else
