...
#ifdef GGML_USE_REEX
    const float inv_std = ggml_cuda_rsqrt_lut_mixed_fp16_reex(var + eps);
#else
...
#ifdef GGML_USE_REEX
    const float scale = ggml_cuda_rsqrt_lut_mixed_fp16_reex(variance + eps);
#else
...
#ifdef GGML_USE_REEX
    const float scale = ggml_cuda_rsqrt_lut_mixed_fp16_reex(mean + eps);
#else
...
#ifdef GGML_USE_REEX
    const float scale_grad = ggml_cuda_rsqrt_lut_mixed_fp16_reex(mean_eps);
#else
...
static __global__ void l2_norm_f32(
    ...
    // from https://pytorch.org/docs/stable/generated/torch.nn.functional.normalize.html
#ifdef GGML_USE_REEX
    const float scale = ggml_cuda_rsqrt_lut_mixed_fp16_reex(fmaxf(tmp, eps * eps));
#else
    const float scale = rsqrtf(fmaxf(tmp, eps * eps));
#endif
...
static __global__ void rms_norm_f16(const half * x, half * dst,
    ...
    for (int col = tid; col < ncols; col += block_size) {
        dst[col] = __float2half(scale * __half2float(x[col]));
    }
}
...
static void rms_norm_f16_cuda(
    ...
    if (ncols < 1024) {
        const dim3 block_dims(256, 1, 1);
        rms_norm_f16<256><<<blocks_num, block_dims, block_dims.x > WARP_SIZE ? 32 * sizeof(float): 0, stream>>>(x, dst, ncols, stride_row, stride_channel, stride_sample, eps);
    } else {
        const dim3 block_dims(1024, 1, 1);
        rms_norm_f16<1024><<<blocks_num, block_dims, block_dims.x > WARP_SIZE ? 32 * sizeof(float): 0, stream>>>(x, dst, ncols, stride_row, stride_channel, stride_sample, eps);
    }
}
