...
#pragma unroll
...
#ifdef GGML_USE_REEX
        const float val = ggml_cuda_exp_for_softmax(vals[col] - max_val);
#else
    ...
    if (sinks) {
#ifdef GGML_USE_REEX
        tmp += ggml_cuda_exp_for_softmax(sinks[i02] - max_val);
#else
...
#pragma unroll
        for (int i = 0; i < n_elem_per_thread; i++) {
            ...
            if (idx < p.ncols) {
#ifdef GGML_USE_REEX
                const float tmp = ggml_cuda_exp_for_softmax(local_vals[i] - local_max);
#else
...
static __global__ void convert_f16_to_f32_softmax(const half * __restrict__ src, float * __restrict__ dst, const int64_t n) {
...
static __global__ void convert_f32_to_f16_softmax(const float * __restrict__ src, half * __restrict__ dst, const int64_t n) {
    const int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
...
void ggml_cuda_op_soft_max(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src0->type == GGML_TYPE_F16) {
        ...
        ggml_cuda_pool_alloc<float> src0_f32_alloc(ctx.pool(), ne);
        ggml_cuda_pool_alloc<float> dst_f32_alloc(ctx.pool(), ne);

        const half * src0_h = (const half *) src0->data;
        float * src0_f = src0_f32_alloc.get();
        float * dst_f  = dst_f32_alloc.get();

        convert_f16_to_f32_softmax<<<(ne + 255)/256, 256, 0, stream>>>(src0_h, src0_f, ne);
        ...
        params.m0          = powf(2.0f, -(max_bias       ) / n_head_log2);
        params.m1          = powf(2.0f, -(max_bias / 2.0f) / n_head_log2);

        if (use_f16_mask) {
            soft_max_f32_cuda(src0_f, (const half *)src1_d, (const float *)src2_d, dst_f, params, stream, ctx);
        } else {
            soft_max_f32_cuda(src0_f, (const float *)src1_d, (const float *)src2_d, dst_f, params, stream, ctx);
        }

        half * dst_h = (half *) dst->data;
        convert_f32_to_f16_softmax<<<(ne + 255)/256, 256, 0, stream>>>(dst_f, dst_h, ne);
        return;
    }
#endif // GGML_REEX_FP16_PIPELINE
