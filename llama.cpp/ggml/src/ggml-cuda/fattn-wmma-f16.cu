...
#pragma unroll
                ...
                KQ_max_scale_f[j0/nwarps] = FATTN_EXPF(diff);
...
#pragma unroll
                for (int k0 = 0; k0 < FATTN_KQ_STRIDE; k0 += warp_size) {
                    ...
                    KQ_f_tmp[k0/warp_size] = FATTN_EXPF(diff);
                    if (diff <= SOFTMAX_FTZ_THRESHOLD) {
...
#pragma unroll
        for (int j0 = 0; j0 < ncols; j0 += nwarps) {
            ...
            if (std::is_same<KQ_acc_t, float>::value) {
                ...
                const float KQ_max_scale = FATTN_EXPF(KQ_max_f[j0/nwarps] - kqmax_new);
                KQ_max_f[j0/nwarps] = kqmax_new;

                KQ_rowsum_f[j0/nwarps] = KQ_rowsum_f[j0/nwarps] * KQ_max_scale + FATTN_EXPF(sinkf - KQ_max_f[j0/nwarps]);
