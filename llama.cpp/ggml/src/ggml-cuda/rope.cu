...
#ifdef GGML_USE_REEX
        mscale *= 1.0f + 0.1f * ggml_cuda_log_lut_mixed_fp16_reex(1.0f / freq_scale);
#else
        mscale *= 1.0f + 0.1f * logf(1.0f / freq_scale);
#endif
    }
#ifdef GGML_USE_REEX
    cos_theta = ggml_cuda_cos_lut_mixed_fp16_reex(theta) * mscale;
    sin_theta = ggml_cuda_sin_lut_mixed_fp16_reex(theta) * mscale;
#else
