...
#ifdef GGML_USE_REEX
        sum += ggml_cuda_exp_for_softmax(logit_i - max_logit);
#else
...
#ifdef GGML_USE_REEX
    sum = ggml_cuda_log_lut_mixed_fp16_reex(sum);
#else
    sum = logf(sum);
#endif
...
static __global__ void cross_entropy_loss_back_f32(
    ...
    for (int i = threadIdx.x; i < nclasses; i += WARP_SIZE) {
#ifdef GGML_USE_REEX
        const float val = ggml_cuda_exp_for_softmax((use_shared ? tmp[i] : logits[i]) - maxval);
#else
