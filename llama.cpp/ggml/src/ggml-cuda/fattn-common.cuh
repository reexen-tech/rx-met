...
#define FATTN_EXPF(x) ggml_cuda_exp_for_softmax(x)
#else
#define FATTN_EXPF(x) expf(x)
#endif

#ifdef REEX_TURBOQUANT
// === REEX_TURBOQUANT BEGIN ===
// FA-vec helpers for TQ_V2 / TQ_V4 (and, in P3.5.b, TQ_K3).
#include "reex/reex_turboquant_fattn_helpers.cuh"
// === REEX_TURBOQUANT END ===
#endif
...
#ifdef REEX_TURBOQUANT
    // === REEX_TURBOQUANT BEGIN ===
    } else if constexpr (type_K == GGML_TYPE_TQ_K3) {
        return vec_dot_fattn_vec_KQ_tq_k3<D, nthreads>;
    // === REEX_TURBOQUANT END ===
#endif
...
#ifdef REEX_TURBOQUANT
    // === REEX_TURBOQUANT BEGIN ===
    } else if constexpr (type_V == GGML_TYPE_TQ_V2) {
        return dequantize_V_tq_v2<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TQ_V4) {
        return dequantize_V_tq_v4<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TQ_V_POLAR2) {
        return dequantize_V_tq_v_polar2<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TQ_V_POLAR4) {
        return dequantize_V_tq_v_polar4<T, ne>;
    // === REEX_TURBOQUANT END ===
#endif
...
static __global__ void flash_attn_combine_results(
    ...
    for (int l = 0; l < parallel_blocks; ++l) {
        const float KQ_max_scale = FATTN_EXPF(meta[l].x - kqmax);
