...
static const struct ggml_type_traits type_traits[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_TQ_K3] = {
        .type_name                = "tq_k3",
        .blck_size                = QK_TQ_K3,
        .type_size                = sizeof(block_tq_k3),
        .is_quantized             = true,
#ifdef REEX_TURBOQUANT
        .to_float                 = (ggml_to_float_t) dequantize_row_tq_k3,
        .from_float_ref           = (ggml_from_float_t) quantize_row_tq_k3_ref,
#endif
    },
    [GGML_TYPE_TQ_V2] = {
        .type_name                = "tq_v2",
        .blck_size                = QK_TQ_V2,
        .type_size                = sizeof(block_tq_v2),
        .is_quantized             = true,
#ifdef REEX_TURBOQUANT
        .to_float                 = (ggml_to_float_t) dequantize_row_tq_v2,
        .from_float_ref           = (ggml_from_float_t) quantize_row_tq_v2_ref,
#endif
    },
    [GGML_TYPE_TQ_V4] = {
        .type_name                = "tq_v4",
        .blck_size                = QK_TQ_V4,
        .type_size                = sizeof(block_tq_v4),
        .is_quantized             = true,
#ifdef REEX_TURBOQUANT
        .to_float                 = (ggml_to_float_t) dequantize_row_tq_v4,
        .from_float_ref           = (ggml_from_float_t) quantize_row_tq_v4_ref,
#endif
    },
    [GGML_TYPE_TQ_V_POLAR2] = {
        .type_name                = "tq_v_polar2",
        .blck_size                = QK_TQ_V_POLAR2,
        .type_size                = sizeof(block_tq_v_polar2),
        .is_quantized             = true,
#ifdef REEX_TURBOQUANT
        .to_float                 = (ggml_to_float_t) dequantize_row_tq_v_polar2,
        .from_float_ref           = (ggml_from_float_t) quantize_row_tq_v_polar2_ref,
#endif
    },
    [GGML_TYPE_TQ_V_POLAR4] = {
        .type_name                = "tq_v_polar4",
        .blck_size                = QK_TQ_V_POLAR4,
        .type_size                = sizeof(block_tq_v_polar4),
        .is_quantized             = true,
#ifdef REEX_TURBOQUANT
        .to_float                 = (ggml_to_float_t) dequantize_row_tq_v_polar4,
        .from_float_ref           = (ggml_from_float_t) quantize_row_tq_v_polar4_ref,
#endif
    },
    // === REEX_TURBOQUANT END ===
};
...
// ggml_reex_wht
//
//   Walsh-Hadamard rotation operator.  Implements
//
//       y = (1/sqrt(g)) * Diag(s2) * H_g * Diag(s1) * (x ⊙ scale_inv)  (forward)
//       y = (1/sqrt(g)) * Diag(s1) * H_g * Diag(s2) * (x ⊙ scale_inv)  (inverse)
//
//   where g = group_size and the s1/s2 sign vectors are the deterministic
//   PRNG-seeded tables baked into the backend's `_signs.inc` (see
//   tools/reex/reex_turboquant_gen_wht_signs.cpp).  The optional `scale_inv`
//   tensor is applied element-wise per channel BEFORE rotation (P4 A2.4
//   Stage 2a moved this from post- to pre-WHT to match InnerQ EMA's
//   inner-product invariance — see ggml.h for math).  Pass NULL to skip.
//
//   The optional `sq_accum` tensor is an F64 per-channel accumulator that
//   the kernel updates with the squared post-rotation outputs:
//       sq_accum[i] += out[i] * out[i]
//   This drives InnerQ EMA's calibration loop (P4 A2.4).  CPU pins to a
//   single thread when sq_accum != NULL.  CUDA returns false from
//   supports_op when sq_accum != NULL (Stage 2a; CUDA accumulator port
//   ships in Stage 4).
//
//   Layout:
//     - `a`           : F32 tensor whose innermost dimension `ne[0]` is a
//                       multiple of `group_size`.  Each contiguous chunk of
//                       `group_size` elements along ne[0] is rotated
//                       independently.
//     - `scale_inv`   : optional 1-D tensor of length `group_size`, F16 or F32.
//     - `sq_accum`    : optional 1-D tensor of length `group_size`, F64.
//
//   Op params (4 i32 lanes used):
//     [0]: direction  (0 = forward, 1 = inverse)
//     [1]: group_size (64 or 128 in Phase 1)
//     [2]: scale_inv_present (0 / 1)
//     [3]: sq_accum_present  (0 / 1)
struct ggml_tensor * ggml_reex_wht(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        int                   direction,
        int                   group_size,
        struct ggml_tensor  * scale_inv,
        struct ggml_tensor  * sq_accum) {
    GGML_ASSERT(a != NULL);
    GGML_ASSERT(a->type == GGML_TYPE_F32);
    GGML_ASSERT(ggml_is_contiguous(a));
    ...
    if (scale_inv != NULL) {
        GGML_ASSERT(ggml_is_contiguous(scale_inv));
        GGML_ASSERT(scale_inv->type == GGML_TYPE_F32 || scale_inv->type == GGML_TYPE_F16);
        GGML_ASSERT(ggml_nelements(scale_inv) == (int64_t) group_size);
    }

    if (sq_accum != NULL) {
        GGML_ASSERT(ggml_is_contiguous(sq_accum));
        // F64 only — the CPU kernel writes through a `double *` view.  The
        // reasonable alternative would be F32 with Kahan summation, but a
        // single F64 buffer of length `group_size` (max 128) is cheap and
        // gives the calibration phase enough headroom to absorb thousands
        // of post-WHT row contributions without precision loss.
        GGML_ASSERT(sq_accum->type == GGML_TYPE_F64);
        GGML_ASSERT(ggml_nelements(sq_accum) == (int64_t) group_size);
    }

    struct ggml_tensor * result = ggml_new_tensor(ctx, a->type, GGML_MAX_DIMS, a->ne);

    ggml_set_op_params_i32(result, 0, (int32_t) direction);
    ggml_set_op_params_i32(result, 1, (int32_t) group_size);
    ggml_set_op_params_i32(result, 2, scale_inv ? 1 : 0);
    ggml_set_op_params_i32(result, 3, sq_accum  ? 1 : 0);

    result->op     = GGML_OP_REEX_WHT;
    result->src[0] = a;
    result->src[1] = scale_inv;
    result->src[2] = sq_accum;
...
#ifdef REEX_TURBOQUANT
        // === REEX_TURBOQUANT BEGIN ===
        case GGML_TYPE_TQ_K3:       result = quantize_tq_k3      (src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
        case GGML_TYPE_TQ_V2:       result = quantize_tq_v2      (src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
        case GGML_TYPE_TQ_V4:       result = quantize_tq_v4      (src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
