...
#ifdef GGML_USE_REEX
    return ggml_sqr_mixed_fp16_f32_REEX(x);
#else
    return x * x;
#endif
}
...
#ifdef GGML_USE_REEX
        return (ggml_exp_lut_mixed_fp16_f32_REEX(min_x_eps) - 1.0f - x) * alpha_n + beta * x;
#else
...
#ifdef GGML_USE_REEX
    return ggml_exp_lut_mixed_fp16_f32_REEX(x) - 1.0f;
#else
    return expf(x) - 1.0f;
#endif
}
...
#ifdef GGML_USE_REEX
    return (x > 20.0f) ? x : ggml_log_lut_mixed_fp16_f32_REEX(1.0f + ggml_exp_lut_mixed_fp16_f32_REEX(x));
#else
