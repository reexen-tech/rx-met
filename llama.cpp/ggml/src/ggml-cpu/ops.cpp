...
#ifdef GGML_USE_REEX
static inline float GGML_EXPF(float x) { return ggml_reex_expf_(x); }
#else
static inline float GGML_EXPF(float x) { return expf(x); }
#endif
...
static void ggml_compute_forward_swiglu_oai_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; k++) {
            ...
            const float out_glu = x / (1.f + GGML_EXPF(alpha * (-x)));
...
#ifdef GGML_USE_REEX
                    const float scale = ggml_rsqrt_lut_mixed_fp16_f32_REEX(mean + eps);
#else
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = ith; i01 < ne01; i01 += nth) {
                ...
                ggml_float sum = 0.0;
                for (int64_t i00 = 0; i00 < ne00; i00++) {
                    float v = GGML_CPU_FP16_TO_FP32(x[i00]);
                    sum += (ggml_float)(v * v);
                }
                ...
                for (int64_t i00 = 0; i00 < ne00; i00++) {
                    float v = GGML_CPU_FP16_TO_FP32(y[i00]) * scale;
                    y[i00] = GGML_CPU_FP32_TO_FP16(v);
                }
            }
        }
    }
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = ith; i01 < ne01; i01 += nth) {
                ...
                ggml_fp16_t * sp = (ggml_fp16_t *)((char *) src0->data + i01*nb01 + i02*nb02 + i03*nb03);
                ...
                for (int i = 0; i < ne00; ++i) {
                    wp[i] = GGML_CPU_FP16_TO_FP32(sp[i]);
                }
                ggml_vec_scale_f32(ne00, wp, scale);
                if (mp_f32) {
                    if (use_f16) {
...
#ifdef GGML_USE_REEX
        mscale *= 1.0f + 0.1f * ggml_log_lut_mixed_fp16_f32_REEX(1.0f / freq_scale);
#else
        mscale *= 1.0f + 0.1f * logf(1.0f / freq_scale);
#endif
    }
#ifdef GGML_USE_REEX
        *cos_theta = ggml_cos_lut_mixed_fp16_f32_REEX(theta) * mscale;
        *sin_theta = ggml_sin_lut_mixed_fp16_f32_REEX(theta) * mscale;
#else
...
#ifdef GGML_USE_REEX
            float freq = (float)GGML_EXPF(-ggml_log_lut_mixed_fp16_f32_REEX((float)max_period) * j / half);
#else
            float freq = (float)GGML_EXPF(-logf(max_period) * j / half);
#endif
            float arg = timestep * freq;
#ifdef GGML_USE_REEX
            embed_data[j] = ggml_cos_lut_mixed_fp16_f32_REEX(arg);
            embed_data[j + half] = ggml_sin_lut_mixed_fp16_f32_REEX(arg);
#else
    ...
    while (ir < ir1) {
        ...
        for (int64_t ic = 0; ic < nek1; ic += KV_TILE_SZ) {
            ...
            for (int tq = 0; tq < Q_TILE_SZ; tq++) {
                ...
                if (Mnew > Mold) {
                    const float ms = GGML_EXPF(Mold - Mnew);
...
static void ggml_flash_attn_ext_reduce_partials(
    ...
    for (int64_t q_head = ith; q_head < n_q_heads; q_head += nth) {
        ...
        for (int64_t chunk_idx = 0; chunk_idx < n_chunks; ++chunk_idx) {
            ...
            const float scale_old = GGML_EXPF(M_final - M_new);
            const float scale_new = GGML_EXPF(M_chunk - M_new);
...
#ifdef GGML_USE_REEX
                    const float dt_soft_plus = (dt[h] > 20.0f) ? dt[h] : ggml_log_lut_mixed_fp16_f32_REEX(1.0f + ggml_exp_lut_mixed_fp16_f32_REEX(dt[h]));
#else
                    const float dt_soft_plus = ggml_compute_softplus_f32(dt[h]);
#endif
                    const float dA = GGML_EXPF(dt_soft_plus * A[h]);
                ...
                for (int h = ih0; h < ih1; ++h) {
                    ...
                    for (int i1 = 0; i1 < nr; ++i1) {
                        ...
                        for (int i0 = 0; i0 < nc; ++i0) {
                            ...
                            const int ig = i0 + g*nc;
                            // state = prev_state * dA + dB * x
                            const float state = (s0[i] * GGML_EXPF(dt_soft_plus * A[i0 + h*nc])) + (B[ig] * x_dt);
                            // y = rowwise_dotprod(state, C)
                            sumf += state * C[ig];
                            s[i] = state;
                        }
