...
inline static float ggml_gelu_quick_f32(float x) {
    ...
    return x*ggml_sigmoid_lut_mixed_fp16_f32_REEX(-GELU_QUICK_COEF*x);
#else
...
#ifdef GGML_USE_REEX
    // Prefer the REEX sigmoid LUT over the precomputed fp16 gelu-quick table.
    for (int i = 0; i < n; ++i) {
...
inline static void ggml_vec_gelu_quick_f16(const int n, ggml_fp16_t * y, const ggml_fp16_t * x) {
    for (int i = 0; i < n; ++i) {
        float v = GGML_CPU_FP16_TO_FP32(x[i]);
#ifdef GGML_USE_REEX
        y[i] = GGML_CPU_FP32_TO_FP16(v*ggml_sigmoid_lut_mixed_fp16_f32_REEX(-GELU_QUICK_COEF*v));
#else
...
#ifdef GGML_USE_REEX
    return GGML_CPU_FP32_TO_FP16(ggml_silu_lut_mixed_fp16_f32_REEX(v));
#else
...
#ifdef GGML_USE_REEX
    const float s = ggml_sigmoid_lut_mixed_fp16_f32_REEX(x);
#else
...
#ifdef GGML_USE_REEX
    const float s = ggml_sigmoid_lut_mixed_fp16_f32_REEX(v);
#else
...
inline static void ggml_vec_swiglu_f16(const int n, ggml_fp16_t * y, const ggml_fp16_t * x, const ggml_fp16_t * g) {
    for (int i = 0; i < n; ++i) {
        ...
        y[i] = GGML_CPU_FP32_TO_FP16(ggml_silu_f32(xi) * gi);
    }
}
...
inline static void ggml_vec_geglu_quick_f16(const int n, ggml_fp16_t * y, const ggml_fp16_t * x, const ggml_fp16_t * g) {
    ...
    for (int i = 0; i < n; ++i) {
        float v = GGML_CPU_FP16_TO_FP32(g[i]);
#ifdef GGML_USE_REEX
        y[i] = GGML_CPU_FP32_TO_FP16(ggml_gelu_quick_f32(GGML_CPU_FP16_TO_FP32(x[i])) * v);
#else
