...
#ifdef GGML_REEX_FP16_PIPELINE
        thread_local std::vector<float> src1_f32_buf;
        const bool src1_is_f16 = (src1->type == GGML_TYPE_F16);
        if (src1_is_f16 && (int64_t)src1_f32_buf.size() < 4 * ne10) {
            src1_f32_buf.resize(4 * ne10);
        }
#endif
            ...
            if (src1_is_f16) {
                for (int64_t i11 = ith * 4; i11 < ne11 - ne11 % 4; i11 += nth * 4) {
                    for (int64_t r = 0; r < 4; r++) {
                        ggml_fp16_to_fp32_row((const ggml_fp16_t *) (data_ptr + (i11 + r) * nb11), src1_f32_buf.data() + r * ne10, ne10);
                    }
                    ggml_quantize_mat_t<INTER_SIZE, PARAM_TYPE>(src1_f32_buf.data(), (void *) (wdata_ptr + i11 * nbw1), 4, ne10);
                }
                const int64_t i11_processed = ne11 - ne11 % 4;
                for (int64_t i11 = i11_processed + ith; i11 < ne11; i11 += nth) {
                    ggml_fp16_to_fp32_row((const ggml_fp16_t *) (data_ptr + i11 * nb11), src1_f32_buf.data(), ne10);
                    from_float(src1_f32_buf.data(), (void *) (wdata_ptr + i11 * nbw1), ne10);
                }
            } else
#endif
            {
...
#ifdef GGML_REEX_FP16_PIPELINE
        thread_local std::vector<float> src1_f32_row_id;
        const bool src1_is_f16_id = (src1->type == GGML_TYPE_F16);
        if (src1_is_f16_id && (int64_t)src1_f32_row_id.size() < ne10) {
            src1_f32_row_id.resize(ne10);
        }
#endif

        // src1: float32 (or F16->F32) => param type
        for (int64_t i12 = 0; i12 < ne12; ++i12) {
            for (int64_t i11 = ith; i11 < ne11; i11 += nth) {
#ifdef GGML_REEX_FP16_PIPELINE
                if (src1_is_f16_id) {
                    ggml_fp16_to_fp32_row((const ggml_fp16_t *)((const char *) src1->data + i12 * nb12 + i11 * nb11), src1_f32_row_id.data(), ne10);
                    from_float(src1_f32_row_id.data(), (void *) (wdata + i12 * nbw2 + i11 * nbw1), ne10);
                } else
#endif
