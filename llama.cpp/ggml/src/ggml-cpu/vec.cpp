...
// when GGML_USE_REEX use exp LUT for softmax (scalar path only; no SIMD exp LUT)
static inline float ggml_soft_max_expf(float v) {
#ifdef GGML_USE_REEX
    return ggml_exp_lut_mixed_fp16_f32_REEX(v);
#else
    return expf(v);
#endif
}
...
ggml_float ggml_vec_soft_max_f32(const int n, float * y, const float * x, float max) {
    ...
    for (; i < n; ++i) {
        float val = ggml_soft_max_expf(x[i] - max);
        y[i] = val;
        sum += (ggml_float)val;
    }
    return sum;
...
ggml_float ggml_vec_log_soft_max_f32(const int n, float * y, const float * x, float max) {
    ...
    for (; i < n; ++i) {
        float val = x[i] - max;
        y[i] = val;
        sum += (ggml_float)ggml_soft_max_expf(val);
    }
#ifdef GGML_USE_REEX
    return sum = (ggml_float)ggml_log_lut_mixed_fp16_f32_REEX((float)sum);
#else
