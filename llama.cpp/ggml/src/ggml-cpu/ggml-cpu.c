...
#if defined(GGML_USE_REEX_GEMM)
#include "reex/reex_gemm_cpu.h"
#endif

#if defined(GGML_USE_REEX)
...
static const struct ggml_type_traits_cpu type_traits_cpu[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_Q4_K] = {
        ...
        /* CPU 口径：
         * - W4×INT8 继续复用原生 Q4_K×Q8_K
         * - 仅 W4×Q16 / W4×F32 切到 REEX 独立 vec_dot
         */
#if defined(GGML_REEX_GEMM_ACTIVATION_FLOAT)
        .vec_dot                  = ggml_vec_dot_q4_K_f32_reex,
        .vec_dot_type             = GGML_TYPE_F32,
        .nrows                    = 1,
#elif defined(GGML_REEX_GEMM_ACTIVATION_Q16)
        .vec_dot                  = ggml_vec_dot_q4_K_q16_reex,
        .vec_dot_type             = GGML_TYPE_I16,
        .nrows                    = 1,
#else
...
#if defined(GGML_REEX_GEMM_ACTIVATION_Q16)
    [GGML_TYPE_I16] = {
        .from_float               = ggml_quantize_row_f32_to_q16_scale_row_reex,
    },
#endif
#ifdef REEX_TURBOQUANT
    // === REEX_TURBOQUANT BEGIN ===
    // TurboQuant KV-cache types are storage-only on the CPU backend in
    // Phase 1 (no `vec_dot`); attention reads them via the rotated
    // flash-attention path defined in Phase 2.  `from_float` forwards to the
    // production `quantize_row_tq_*` (which currently aliases the `_ref`).
    [GGML_TYPE_TQ_K3] = {
        .from_float               = quantize_row_tq_k3,
        .vec_dot                  = ggml_vec_dot_tq_k3_f32,
        .vec_dot_type             = GGML_TYPE_F32,
        .nrows                    = 1,
    },
    [GGML_TYPE_TQ_V2] = {
        .from_float               = quantize_row_tq_v2,
        .vec_dot                  = ggml_vec_dot_tq_v2_f32,
        .vec_dot_type             = GGML_TYPE_F32,
        .nrows                    = 1,
    },
    [GGML_TYPE_TQ_V4] = {
        .from_float               = quantize_row_tq_v4,
        .vec_dot                  = ggml_vec_dot_tq_v4_f32,
        .vec_dot_type             = GGML_TYPE_F32,
        .nrows                    = 1,
    },
    [GGML_TYPE_TQ_V_POLAR2] = {
        .from_float               = quantize_row_tq_v_polar2,
        .vec_dot                  = ggml_vec_dot_tq_v_polar2_f32,
        .vec_dot_type             = GGML_TYPE_F32,
        .nrows                    = 1,
    },
    [GGML_TYPE_TQ_V_POLAR4] = {
        .from_float               = quantize_row_tq_v_polar4,
        .vec_dot                  = ggml_vec_dot_tq_v_polar4_f32,
        .vec_dot_type             = GGML_TYPE_F32,
        .nrows                    = 1,
    },
    // === REEX_TURBOQUANT END ===
...
#if defined(GGML_USE_REEX_GEMM)
    const size_t row_size = (vec_dot_type == GGML_TYPE_I16)
        ? ggml_q16_activation_row_size_reex(ne10)
...
void ggml_compute_forward_mul_mat(
    ...
    ggml_vec_dot_t    the_vec_dot;
    enum ggml_type    the_vec_dot_type;
    ggml_from_float_t the_from_float;
    int64_t           the_vec_dot_num_rows;

#if defined(GGML_REEX_GEMM_ACTIVATION_Q16) || defined(GGML_REEX_GEMM_ACTIVATION_FLOAT)
    {
        const int target = ggml_reex_get_target_layer();
        bool use_reex = true;
        if (target >= 0 && src0->type == GGML_TYPE_Q4_K) {
            int tl = ggml_reex_tensor_layer(dst->name);
            use_reex = (tl == target);
        }
        if (use_reex && src0->type == GGML_TYPE_Q4_K) {
#if defined(GGML_REEX_GEMM_ACTIVATION_Q16)
            the_vec_dot          = ggml_vec_dot_q4_K_q16_reex;
            the_vec_dot_type     = GGML_TYPE_I16;
            the_from_float       = ggml_quantize_row_f32_to_q16_scale_row_reex;
#else
            the_vec_dot          = ggml_vec_dot_q4_K_f32_reex;
            the_vec_dot_type     = GGML_TYPE_F32;
            the_from_float       = NULL;
#endif
            the_vec_dot_num_rows = 1;
        } else {
            the_vec_dot          = (src0->type == GGML_TYPE_Q4_K && !use_reex)
                                     ? ggml_vec_dot_q4_K_q8_K
                                     : type_traits_cpu[src0->type].vec_dot;
            the_vec_dot_type     = (src0->type == GGML_TYPE_Q4_K && !use_reex)
                                     ? GGML_TYPE_Q8_K
                                     : type_traits_cpu[src0->type].vec_dot_type;
            the_from_float       = type_traits_cpu[the_vec_dot_type].from_float;
            the_vec_dot_num_rows = (src0->type == GGML_TYPE_Q4_K && !use_reex)
                                     ? 1
                                     : type_traits_cpu[src0->type].nrows;
        }
    }
#else
    the_vec_dot          = type_traits_cpu[src0->type].vec_dot;
    the_vec_dot_type     = type_traits_cpu[src0->type].vec_dot_type;
    the_from_float       = type_traits_cpu[the_vec_dot_type].from_float;
    the_vec_dot_num_rows = type_traits_cpu[src0->type].nrows;
#endif

    const enum ggml_type    vec_dot_type     = the_vec_dot_type;
    const ggml_from_float_t from_float       = the_from_float;
    const int64_t           vec_dot_num_rows = the_vec_dot_num_rows;
...
#if defined(GGML_REEX_GEMM_ACTIVATION_Q16) || defined(GGML_REEX_GEMM_ACTIVATION_FLOAT)
    if (vec_dot_type != GGML_TYPE_I16 && vec_dot_type != GGML_TYPE_F32) {
#endif
    if (src1_cont) {
...
UseGgmlGemm1:;
    ...
    if (src1->type != vec_dot_type) {
...
#if defined(GGML_USE_REEX_GEMM)
        const size_t nbw1 = (vec_dot_type == GGML_TYPE_I16)
            ? ggml_q16_activation_row_size_reex(ne10)
            : ggml_row_size(vec_dot_type, ne10);
        const size_t nbw0 = (vec_dot_type == GGML_TYPE_I16) ? 0 : ggml_type_size(vec_dot_type);
#else
        ...
        float * src1_f32_tmp = NULL;
        if (src1->type == GGML_TYPE_F16) {
            src1_f32_tmp = (float *) malloc(ne10 * sizeof(float));
            GGML_ASSERT(src1_f32_tmp != NULL);
        }
#else
...
#if defined(GGML_USE_REEX_GEMM)
        if (vec_dot_type == GGML_TYPE_I16) {
            for (int64_t i13 = 0; i13 < ne13; ++i13) {
                for (int64_t i12 = 0; i12 < ne12; ++i12) {
                    for (int64_t i11 = ith; i11 < ne11; i11 += nth) {
                        const void * src1_row = (char *) src1->data + i13*nb13 + i12*nb12 + i11*nb11;
#ifdef GGML_REEX_FP16_PIPELINE
                        const float * src1_f32;
                        if (src1->type == GGML_TYPE_F16) {
                            ggml_fp16_to_fp32_row((const ggml_fp16_t *)src1_row, src1_f32_tmp, ne10);
                            src1_f32 = src1_f32_tmp;
                        } else {
                            src1_f32 = (const float *)src1_row;
                        }
                        from_float(src1_f32,
                                  (void *) (wdata + i13*nbw3 + i12*nbw2 + i11*nbw1),
                                  ne10);
#else
                        from_float((float *)src1_row,
                                  (void *) (wdata + i13*nbw3 + i12*nbw2 + i11*nbw1),
                                  ne10);
#endif
                    }
...
#ifdef GGML_REEX_FP16_PIPELINE
                    if (src1->type == GGML_TYPE_F16) {
                        const int64_t n_elems = (ne10_block_end - ne10_block_start) * bs;
                        const ggml_fp16_t * src1_fp16 = (const ggml_fp16_t *)((char *) src1->data + i13*nb13 + i12*nb12 + i11*nb11) + ne10_block_start*bs;
                        ggml_fp16_to_fp32_row(src1_fp16, src1_f32_tmp, n_elems);
                        from_float(src1_f32_tmp,
...
UseGgmlGemm2:;
    ...
    while (current_chunk < nchunk0 * nchunk1) {
        ...
        ggml_compute_forward_mul_mat_one_chunk(params, dst, the_vec_dot, vec_dot_type, num_rows_per_vec_dot, ir0_start, ir0_end, ir1_start, ir1_end);
...
static void ggml_compute_forward_mul_mat_id(
...
#if defined(GGML_REEX_GEMM_ACTIVATION_Q16) || defined(GGML_REEX_GEMM_ACTIVATION_FLOAT)
    {
        const int target = ggml_reex_get_target_layer();
        bool use_reex = true;
        if (target >= 0 && type == GGML_TYPE_Q4_K) {
            int tl = ggml_reex_tensor_layer(dst->name);
            use_reex = (tl == target);
        }
        if (use_reex && type == GGML_TYPE_Q4_K) {
#if defined(GGML_REEX_GEMM_ACTIVATION_Q16)
            id_vec_dot   = ggml_vec_dot_q4_K_q16_reex;
            vec_dot_type = GGML_TYPE_I16;
            from_float   = ggml_quantize_row_f32_to_q16_scale_row_reex;
#else
            id_vec_dot   = ggml_vec_dot_q4_K_f32_reex;
            vec_dot_type = GGML_TYPE_F32;
            from_float   = NULL;
#endif
        } else {
            id_vec_dot   = (type == GGML_TYPE_Q4_K && !use_reex)
                             ? ggml_vec_dot_q4_K_q8_K
                             : type_traits_cpu[type].vec_dot;
            vec_dot_type = (type == GGML_TYPE_Q4_K && !use_reex)
                             ? GGML_TYPE_Q8_K
                             : type_traits_cpu[type].vec_dot_type;
            from_float   = type_traits_cpu[vec_dot_type].from_float;
        }
    }
#else
    id_vec_dot   = type_traits_cpu[type].vec_dot;
    vec_dot_type = type_traits_cpu[type].vec_dot_type;
    from_float   = type_traits_cpu[vec_dot_type].from_float;
#endif
    ...
    if (src1->type != vec_dot_type) {
        ...
        if (vec_dot_type == GGML_TYPE_I16) {
            incr_ptr_aligned(&wdata_cur,
                ne12 * ne13 * ne11 * ggml_q16_activation_row_size_reex(ne10), sizeof(int64_t));
        } else
#endif
    ...
    for (int cur_a = 0; cur_a < n_as; ++cur_a) {
        ...
        while (current_chunk < nchunk0 * nchunk1) {
            ...
            ggml_compute_forward_mul_mat_id_one_chunk(
                dst, src0, src1, ids, id_vec_dot, vec_dot_type, cur_a,
                ir0_start, ir0_end, ir1_start, ir1_end,
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
...
#if defined(GGML_REEX_GEMM_ACTIVATION_Q16) || defined(GGML_REEX_GEMM_ACTIVATION_FLOAT)
    {
        ...
        if ((tensor->op == GGML_OP_MUL_MAT || tensor->op == GGML_OP_MUL_MAT_ID) &&
            tensor->src[0]->type == GGML_TYPE_Q4_K) {
            const int target = ggml_reex_get_target_layer();
            if (target < 0 || ggml_reex_tensor_layer(tensor->name) == target) {
                skip_extra = true;
            }
        }
        if (!skip_extra && ggml_cpu_extra_compute_forward(params, tensor)) {
            return;
        }
    }
#else
    ...
    switch (tensor->op) {
...
#ifdef REEX_TURBOQUANT
        // === REEX_TURBOQUANT BEGIN ===
        case GGML_OP_REEX_WHT:
            {
                const int direction    = ggml_get_op_params_i32(tensor, 0);
                const int group_size   = ggml_get_op_params_i32(tensor, 1);
                const int has_scale    = ggml_get_op_params_i32(tensor, 2);
                const int has_sq_accum = ggml_get_op_params_i32(tensor, 3);

                const struct ggml_tensor * src0 = tensor->src[0];
                const struct ggml_tensor * src1 = has_scale    ? tensor->src[1] : NULL;
                const struct ggml_tensor * src2 = has_sq_accum ? tensor->src[2] : NULL;
                ...
                // scale_inv is at most group_size==128 f32 entries, so a small
                // stack buffer is plenty.  The buffer is unused when src1==NULL.
                float        scale_buf[128];
                const float * scale_ptr = NULL;
                if (src1 != NULL) {
                    GGML_ASSERT(group_size <= (int) (sizeof(scale_buf) / sizeof(scale_buf[0])));
                    if (src1->type == GGML_TYPE_F32) {
                        scale_ptr = (const float *) src1->data;
                    } else {
                        const ggml_fp16_t * h = (const ggml_fp16_t *) src1->data;
                        for (int g = 0; g < group_size; ++g) {
                            scale_buf[g] = GGML_FP16_TO_FP32(h[g]);
                        }
                        scale_ptr = scale_buf;
                    }
                }

                double * sq_accum_ptr = NULL;
                if (src2 != NULL) {
                    GGML_ASSERT(src2->type == GGML_TYPE_F64);
                    GGML_ASSERT(ggml_nelements(src2) == (int64_t) group_size);
                    sq_accum_ptr = (double *) src2->data;
                }
                ...
                if (sq_accum_ptr != NULL) {
                    // Single-threaded path (n_tasks==1 contract from
                    // ggml_get_n_tasks below).  We process ALL rows on
                    // ith==0 and the kernel writes serially into sq_accum.
                    // The contract is enforced upstream — assert here so
                    // any scheduler regression is caught loudly.
                    GGML_ASSERT(nth == 1);
                    if (ith == 0 && rows > 0) {
                        reex_turboquant_cpu_wht_apply_rows(
                            src, dst, rows, group_size, direction,
                            scale_ptr, sq_accum_ptr);
                    }
                } else {
                    // Row-parallel split: each thread takes a contiguous
                    // slice.  No sq_accum → no shared writes.
        ...
        case GGML_OP_REEX_WHT:
            {
                GGML_ABORT("GGML_OP_REEX_WHT requires REEX_TURBOQUANT");
            } break;
#endif
        case GGML_OP_NONE:
            {
...
static int ggml_get_n_tasks(struct ggml_tensor * node, int n_threads) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_REEX_WHT:
            {
                // Pin to a single task when sq_accum is non-NULL so the per-
                // channel accumulator update (which is shared across rows)
                // stays serial without needing atomics.  This is the InnerQ
                // EMA calibration path, which only runs for the first
                // ~1024 prefill rows per layer — the perf cost of dropping
                // to 1 thread for those rows is negligible compared to the
                // amortised cost of the rest of the prefill.
                const int has_sq_accum = ggml_get_op_params_i32(node, 3);
                n_tasks = has_sq_accum ? 1 : n_threads;
            } break;
        // === REEX_TURBOQUANT END ===
#else
        case GGML_OP_REEX_WHT:
...
struct ggml_cplan ggml_graph_plan(
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        {
#if defined(GGML_REEX_GEMM_ACTIVATION_Q16) || defined(GGML_REEX_GEMM_ACTIVATION_FLOAT)
        bool skip_extra_ws = false;
        if ((node->op == GGML_OP_MUL_MAT || node->op == GGML_OP_MUL_MAT_ID) &&
            node->src[0]->type == GGML_TYPE_Q4_K) {
            const int target = ggml_reex_get_target_layer();
            if (target < 0 || ggml_reex_tensor_layer(node->name) == target) {
                skip_extra_ws = true;
            }
        }
        if (skip_extra_ws || !ggml_cpu_extra_work_size(n_threads, node, &cur))
#else
...
#endif
        {
            switch (node->op) {
                ...
                case GGML_OP_MUL_MAT:
                    {
                        enum ggml_type vec_dot_type = type_traits_cpu[node->src[0]->type].vec_dot_type;

                        if (node->src[1]->type != vec_dot_type) {
#if defined(GGML_USE_REEX_GEMM)
                            if (vec_dot_type == GGML_TYPE_I16) {
                                const int64_t ne10 = node->src[1]->ne[0], ne11 = node->src[1]->ne[1];
                                const int64_t ne12 = node->src[1]->ne[2], ne13 = node->src[1]->ne[3];
                                cur = ne12 * ne13 * ne11 * ggml_q16_activation_row_size_reex(ne10);
                            } else
#endif
                        ...
                        if (node->src[0]->type == GGML_TYPE_Q4_K) {
                            size_t q8k_ws = ggml_row_size(GGML_TYPE_Q8_K, ggml_nelements(node->src[1]));
                            if (q8k_ws > cur) cur = q8k_ws;
                        }
#endif
                    } break;
                case GGML_OP_MUL_MAT_ID:
                    {
                        ...
                        if (src1->type != vec_dot_type) {
                            ...
                            if (vec_dot_type == GGML_TYPE_I16) {
                                ...
                                cur += ne12 * ne13 * ne11 * ggml_q16_activation_row_size_reex(ne10) + sizeof(int64_t);
                            } else
#endif
                        ...
                        if (src0->type == GGML_TYPE_Q4_K) {
                            size_t q8k_ws = ggml_row_size(GGML_TYPE_Q8_K, ggml_nelements(src1)) + sizeof(int64_t);
                            if (q8k_ws > cur) cur = q8k_ws;
                        }
#endif
                        // matrix_row_counts
...
#ifdef GGML_USE_REEX
            // Initialize trigonometric LUT tables (shared with CUDA; in ggml-base)
