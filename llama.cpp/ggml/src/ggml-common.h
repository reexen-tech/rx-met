...
// TurboQuant KV-cache compression block layouts.  See
// docs/turboquant/02_data_layout.md for the canonical bit-level spec.
//
// K cache: rotated via the WHT op, then 3-bit polar-quant per element with
// QJL residual sign bits.  One block packs `QK_TQ_K3 = 128` elements (i.e.
// one head_dim row) so that the WHT inside ggml_reex_wht stays per-block.
#define QK_TQ_K3 128
typedef struct {
    ggml_half norm;                        // L2 norm of the rotated vector
    uint8_t   qs   [QK_TQ_K3 / 4];         // 3-bit indices (4-bit nibble container, LSB-first)
    uint8_t   signs[QK_TQ_K3 / 8];         // QJL residual sign bits, LSB-first
} block_tq_k3;
static_assert(sizeof(block_tq_k3) == sizeof(ggml_half) + QK_TQ_K3/4 + QK_TQ_K3/8, "wrong block_tq_k3 size/padding");

// V cache (legacy double-track, P2 phase): rotated via V WHT (same group=128
// WHT as K cache, applied in src/llama-kv-cache.cpp::cpy_v under tq_v_active),
// then per-block min/max scalar quantization with `block == group == 32`,
// matching ggml's existing q4_0 / q5_0 / q8_0 group convention.
//
// NOTE (2026-05-11 stale fact correction): earlier comments here described V
// cache as "not rotated"; that was a doc/code drift bug — V WHT has been
// present since P2 phase land. See docs/turboquant/02_data_layout.md §1 end
// "V WHT status (factual correction, 2026-05-11)" and
// docs/turboquant/06_reference_diff_and_tradeoffs.md §2.5 末 事实勘正注.
#define QK_TQ_V2 32
typedef struct {
    ggml_half scale;                       // per-block scale (min/max derived)
    ggml_half zero;                        // per-block zero point
    uint8_t   qs[QK_TQ_V2 / 4];            // 2-bit indices, 4 elt/byte (LSB-first)
} block_tq_v2;
static_assert(sizeof(block_tq_v2) == 2*sizeof(ggml_half) + QK_TQ_V2/4, "wrong block_tq_v2 size/padding");

#define QK_TQ_V4 32
typedef struct {
    ggml_half scale;
    ggml_half zero;
    uint8_t   qs[QK_TQ_V4 / 2];            // 4-bit indices, 2 elt/byte (LSB-first)
} block_tq_v4;
static_assert(sizeof(block_tq_v4) == 2*sizeof(ggml_half) + QK_TQ_V4/2, "wrong block_tq_v4 size/padding");

// V cache (P4 A2.5 V-β, 2026-05-11+): rotated via V WHT (same group=128 WHT
// as K cache and as legacy tq_v2/tq_v4 above; activation predicate
// tq_v_active is extended to include V_POLAR2/V_POLAR4 in src/llama-kv-cache.cpp::cpy_v
// and src/llama-graph.cpp::build_attn_mha), then PolarQuant scalar
// quantization with Lloyd-Max codebook (4 centroids @ 2-bit /
// 16 centroids @ 4-bit, N(0,1)-optimal) plus norm correction trick that
// preserves ‖dequant(q)‖ ≡ ‖input‖ (strict idempotency).
//
// Block size = 128 elements (one head_dim row) so that WHT inside
// ggml_reex_wht and quantize stay per-block, and so that the byte layout is
// bit-level compatible with TheTom turbo2_0/turbo4_0 (llama-cpp-turboquant).
//
// rnorm field in block_tq_v_polar4 is reserved (always written as 0) for
// byte-level alignment with TheTom turbo4_0 layout; V-β does not use it.
// See docs/turboquant/p4.a25_v_polar_design.md §3 for the full design spec.
#define QK_TQ_V_POLAR2 128
typedef struct {
    ggml_half norm;                        // corrected = ‖x‖ / ‖recon_unit‖
    uint8_t   qs[QK_TQ_V_POLAR2 / 4];      // 2-bit Lloyd-Max indices, 4 elt/byte (LSB-first)
} block_tq_v_polar2;
static_assert(sizeof(block_tq_v_polar2) == sizeof(ggml_half) + QK_TQ_V_POLAR2/4, "wrong block_tq_v_polar2 size/padding");
static_assert(sizeof(block_tq_v_polar2) == 34, "block_tq_v_polar2 must be 34 bytes to match TheTom turbo2_0 bit layout");

#define QK_TQ_V_POLAR4 128
typedef struct {
    ggml_half norm;                        // corrected = ‖x‖ / ‖recon_unit‖
    ggml_half rnorm;                       // reserved (always 0); kept for TheTom turbo4_0 byte alignment
    uint8_t   qs[QK_TQ_V_POLAR4 / 2];      // 4-bit Lloyd-Max indices, 2 elt/byte (LSB-first)
} block_tq_v_polar4;
static_assert(sizeof(block_tq_v_polar4) == 2*sizeof(ggml_half) + QK_TQ_V_POLAR4/2, "wrong block_tq_v_polar4 size/padding");
static_assert(sizeof(block_tq_v_polar4) == 68, "block_tq_v_polar4 must be 68 bytes to match TheTom turbo4_0 bit layout");
// === REEX_TURBOQUANT END ===
