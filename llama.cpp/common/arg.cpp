...
const std::vector<ggml_type> kv_cache_types = {
    ...
    // P3.6 / P4 A2.5 V-β: expose TurboQuant K/V types to the -ctk / -ctv CLI
    // so quality_gate and llama-bench can run them as a peer of q8_0 / f16
    // etc. The values accepted via the CLI are the same `ggml_type_name`
    // strings (see ggml.c type_traits: "tq_k3", "tq_v2", "tq_v4",
    // "tq_v_polar2", "tq_v_polar4"). Only meaningful K/V uses are:
    //   K: tq_k3
    //   V (legacy):    tq_v2 / tq_v4 (scale+zero, group=32, P3.4 land)
    //   V (V-β):       tq_v_polar2 / tq_v_polar4 (Lloyd-Max + norm correction,
    //                  group=128, P4 A2.5 land, TheTom turbo2_0 / turbo4_0
    //                  algorithmic alignment).
    // Any other combination falls through to the existing kernel allow-lists
    // (FA-vec dispatch in fattn.cu rejects mismatched K/V at runtime).
    GGML_TYPE_TQ_K3,
    GGML_TYPE_TQ_V2,
    GGML_TYPE_TQ_V4,
...
#ifdef REEX_TURBOQUANT
    // === REEX_TURBOQUANT BEGIN ===
    // TurboQuant KV-cache compression knobs.  See docs/turboquant/01_设计与实现计划.md.
    // Whether TQ engages is decided by `-ctk` / `-ctv` (e.g. `-ctk tq_k3
    // -ctv tq_v2`); the knobs below are orthogonal user-facing semantic
    // dimensions kept for forward extensibility.
    add_opt(common_arg(
        {"--tq-key-bits"}, "N",
        string_format("TurboQuant K-cache bits (currently only 3 is supported -> TQ_K3) (default: %d)",
                      params.tq_key_bits),
        [](common_params & params, int value) {
            params.tq_key_bits = value;
        }
    ).set_env("LLAMA_ARG_TQ_KEY_BITS"));
    add_opt(common_arg(
        {"--tq-value-bits"}, "N",
        string_format("TurboQuant V-cache bits (2 -> TQ_V2 or 4 -> TQ_V4) (default: %d)",
                      params.tq_value_bits),
        [](common_params & params, int value) {
            params.tq_value_bits = value;
        }
    ).set_env("LLAMA_ARG_TQ_VALUE_BITS"));
    add_opt(common_arg(
        {"--tq-seed"}, "N",
        string_format("TurboQuant PRNG seed (currently runtime no-op; WHT signs / Lloyd-Max codebook are build-time) (default: %d)",
                      params.tq_seed),
        [](common_params & params, int value) {
            params.tq_seed = value;
        }
    ).set_env("LLAMA_ARG_TQ_SEED"));
    add_opt(common_arg(
        {"--tq-innerq"},
        {"--no-tq-innerq"},
        string_format("enable TurboQuant InnerQ EMA per-channel scale (P4 A2.4 plumbing; currently no-op) (default: %s)",
                      params.tq_innerq_enable ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.tq_innerq_enable = value;
        }
    ).set_env("LLAMA_ARG_TQ_INNERQ"));
    // === REEX_TURBOQUANT END ===
#endif
    add_opt(common_arg(
        {"--hellaswag"},
