...
struct common_params {
    ...
    // TurboQuant orthogonal knobs.  Whether TQ engages is decided by
    // cache_type_k / cache_type_v above (set via `-ctk` / `-ctv`).
    int32_t tq_key_bits        = 3;     // currently only 3 -> TQ_K3 implemented
    int32_t tq_value_bits      = 2;     // 2 -> TQ_V2 or 4 -> TQ_V4
    int32_t tq_seed            = 42;    // currently runtime no-op (build-time codebook)
    bool    tq_innerq_enable   = true;  // P4 A2.4 plumbing; currently no-op
    // === REEX_TURBOQUANT END ===
#endif
